...
def preprocess():
    ...
    #print (type(x_text))                                                     list
    #print (type(y))                                                          numpy.ndarray

    # Build vocabulary
    max_document_length = max([len(x.split(" ")) for x in x_text])             #56
    vocab_processor = learn.preprocessing.VocabularyProcessor(max_document_length)
    #print (type(vocab_processor))
    x = np.array(list(vocab_processor.fit_transform(x_text)))                   #10662 * 56

    # Randomly shuffle data
    np.random.seed(10)
    shuffle_indices = np.random.permutation(np.arange(len(y)))
    print (shuffle_indices[0:3])
    ...
    x_train, x_dev = x_shuffled[:dev_sample_index], x_shuffled[dev_sample_index:]  #9596 * 56   1066 * 56
    y_train, y_dev = y_shuffled[:dev_sample_index], y_shuffled[dev_sample_index:]  #9596 * 2    9596 * 2
    print (x_train[2])
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            for batch in batches:
                #print (batch[0])
                x_batch, y_batch = zip(*batch)        #x_batch = 64 * 56  y_batch = 64 * 2
